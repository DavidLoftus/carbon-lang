...
// Forward declaration.
static auto CanConvertAggregateElement(Context& context,
                                       SemIR::TypeId src_elem_type,
                                       ConversionTarget::Kind kind,
                                       SemIR::TypeId target_elem_type,
                                       SemIR::ClassType* vtable_class_type)
    -> bool;

static auto CanConvertTupleToArray(Context& context,
                                   SemIR::TupleType tuple_type,
  ...
  for (auto src_type_inst_id :
       context.types().GetBlockAsTypeInstIds(tuple_elem_types)) {
    auto src_elem_type =
        context.types().GetTypeIdForTypeInstId(src_type_inst_id);
    if (!CanConvertAggregateElement(context, src_elem_type,
...
static auto CanConvertTupleToTuple(Context& context, SemIR::TupleType src_type,
                                   SemIR::TupleType dest_type,
  ...
  for (auto [src_type_inst_id, dest_type_inst_id] :
       llvm::zip(context.types().GetBlockAsTypeInstIds(src_elem_types),
                 context.types().GetBlockAsTypeInstIds(dest_elem_types))) {
    auto src_elem_type =
        context.types().GetTypeIdForTypeInstId(src_type_inst_id);
    auto dest_elem_type =
        context.types().GetTypeIdForTypeInstId(dest_type_inst_id);
    if (!CanConvertAggregateElement(context, src_elem_type, inner_kind,
                                    dest_elem_type, nullptr)) {
      return false;
    }
  }
  return true;
...
static auto CanConvertStructToStructOrClass(
    Context& context, SemIR::StructType src_type, SemIR::StructType dest_type,
    ConversionTarget target, SemIR::ClassType* vtable_class_type) -> bool {
  auto& sem_ir = context.sem_ir();
  auto src_elem_fields = sem_ir.struct_type_fields().Get(src_type.fields_id);
  auto dest_elem_fields = sem_ir.struct_type_fields().Get(dest_type.fields_id);
  ...
  if (src_type.fields_id != dest_type.fields_id) {
    for (auto [i, field] : llvm::enumerate(src_elem_fields)) {
      src_field_indexes.Insert(field.name_id, i);
    }
  }
  ...
  for (auto [i, dest_field] : llvm::enumerate(dest_elem_fields)) {
    ...
    auto src_elem_type =
        context.types().GetTypeIdForTypeInstId(src_field.type_inst_id);
    auto dest_elem_type =
        context.types().GetTypeIdForTypeInstId(dest_field.type_inst_id);

    if (!CanConvertAggregateElement(context, src_elem_type, inner_kind,
                                    dest_elem_type, vtable_class_type)) {
      return false;
    }
  }
  return true;
...
static auto CanConvertStructToStruct(Context& context,
                                     SemIR::StructType src_type,
                                     SemIR::StructType dest_type,
                                     ConversionTarget target) -> bool {
  return CanConvertStructToStructOrClass(context, src_type, dest_type, target,
                                         nullptr);
}

static auto CanConvertStructToClass(
  ...
  return CanConvertStructToStructOrClass(
      context, src_type, dest_struct_type, target,
      vtable_class_type ? vtable_class_type : &dest_type);
}
...
static auto CanConvertAggregateElement(Context& context,
    ...
    -> bool {
  ConversionTarget target = {.kind = kind, .type_id = target_elem_type};
  return CanPerformBuiltinConversion(context, src_elem_type, target,
                                     vtable_class_type);
}

auto CanPerformBuiltinConversion(Context& context, SemIR::TypeId value_type_id,
                                 ConversionTarget target,
                                 SemIR::ClassType* vtable_class_type) -> bool {
  auto& sem_ir = context.sem_ir();
  ...
  if (target_foundation_id == value_foundation_id) {
    // We assume qualifiers are compatible if the types are compatible,
    // ignoring category restrictions as requested.
    return true;
  }
  ...
  if (auto target_tuple_type = target_type_inst.TryAs<SemIR::TupleType>()) {
    if (auto src_tuple_type =
      ...
      return CanConvertTupleToTuple(context, *src_tuple_type,
  ...
  if (auto target_struct_type = target_type_inst.TryAs<SemIR::StructType>()) {
    if (auto src_struct_type =
      ...
      return CanConvertStructToStruct(context, *src_struct_type,
                                      *target_struct_type, target);
    }
  }
  ...
  if (auto target_array_type = target_type_inst.TryAs<SemIR::ArrayType>()) {
    if (auto src_tuple_type =
            sem_ir.types().TryGetAs<SemIR::TupleType>(value_type_id)) {
      return CanConvertTupleToArray(context, *src_tuple_type,
                                    *target_array_type);
    }
  }
  ...
  if (auto target_class_type = target_type_inst.TryAs<SemIR::ClassType>()) {
    if (auto src_struct_type =
      ...
      if (!context.classes()
               ...
               .adapt_id.has_value()) {
        return CanConvertStructToClass(context, *src_struct_type,
    ...
    if (auto path =
            ComputeInheritancePath(context, SemIR::LocId(SemIR::InstId::None),
  ...
  if (auto target_pointer_type = target_type_inst.TryAs<SemIR::PointerType>()) {
    if (auto src_pointer_type =
      ...
      TryToCompleteType(context, target_pointee_id,
                        SemIR::LocId(SemIR::InstId::None));
      TryToCompleteType(context, src_pointee_id,
                        SemIR::LocId(SemIR::InstId::None));
      ...
      if (unqual_target_pointee_type_id != unqual_src_pointee_type_id) {
        if (auto path = ComputeInheritancePath(
                context, SemIR::LocId(SemIR::InstId::None),
                unqual_src_pointee_type_id, unqual_target_pointee_type_id);
            path && !path->empty()) {
          return true;
        } else {
          return false;
  ...
  if (sem_ir.types().IsFacetType(target.type_id)) {
    if (sem_ir.types().Is<SemIR::TupleType>(value_type_id)) {
      // We can't easily check if a tuple value is a tuple of types without the
      // value. Conservative false.
      return false;
    }
    ...
    // We can't easily check if a type value implements a facet without the
    // value. Conservative false.
    return false;
  }
